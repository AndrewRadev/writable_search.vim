...
describe "Parsing" do
  before :each do
    FileUtils.mkdir_p('autoload/writable_search')

    write_file 'autoload/writable_search/proxy.vim', <<~EOF
      " padding
      " to adjust next proxies.
      function! writable_search#proxy#UpdateSource(new_lines, adjustment) dict

    EOF
    write_file 'autoload/writable_search/parser.vim', <<~EOF
  ...
  it "parses grep results into a different representation (separated by dashes)" do
    write_file 'grep_results', <<~EOF
      autoload/writable_search/proxy.vim-2-" to adjust next proxies.
      autoload/writable_search/proxy.vim:3:function! writable_search#proxy#UpdateSource(new_lines, adjustment) dict
      autoload/writable_search/proxy.vim-4-
      --
      ...
      --
      autoload/writable_search/parser.vim-5-  for lines in a:grouped_lines
      autoload/writable_search/parser.vim:6:    let current_proxy          = writable_search#proxy#New(bufnr('%'))
      autoload/writable_search/parser.vim-7-    let current_proxy.filename = s:FindFilename(lines)
    EOF
    ...
    expect(vim.buffer_contents.lines.map(&:strip)).to eq <<~EOF.lines.map(&:strip)
      autoload/writable_search/proxy.vim:2-4
       " to adjust next proxies.
       function! writable_search#proxy#UpdateSource(new_lines, adjustment) dict
      ...
      autoload/writable_search/parser.vim:1-2
       function! writable_search#parser#Run()
         let grouped_lines = s:PartitionLines(getbufline('%', 1, '$'))
      autoload/writable_search/parser.vim:5-7
         for lines in a:grouped_lines
           let current_proxy          = writable_search#proxy#New(bufnr('%'))
           let current_proxy.filename = s:FindFilename(lines)
    EOF
